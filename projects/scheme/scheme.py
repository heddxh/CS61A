...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args: Pair, env):
        ...
        try:
            ...
            raise SchemeError(f"incorrect number of arguments: {self}")
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        names = scheme_append(names, Pair(binding.first, nil))
        values = scheme_append(values, Pair(scheme_eval(binding.rest.first, env), nil))
...
def validate_procedure(procedure: Procedure):
    ...
    if not scheme_procedurep(procedure):
        raise SchemeError(
            f"{type(procedure).__name__.lower()} is not callable: {repl_str(procedure)}"
        )
...
def scheme_load(*args):
    ...
    if not (2 <= len(args) <= 3):
        expressions = args[:-1]
        raise SchemeError('"load" given incorrect number of arguments: ' f"{len(expressions)}")
    sym = args[0]
...
def run(*argv):
    ...
    parser.add_argument(
        "--turtle-save-path", default=None, help="save the image to this location when done"
    )
    parser.add_argument("-load", "-i", action="store_true", help="run file interactively")
    parser.add_argument(
        "file", nargs="?", type=argparse.FileType("r"), default=None, help="Scheme file to run"
    )
