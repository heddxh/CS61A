...
GOAL_SCORE = 100  # The goal of Hog is to score 100 points.
                 # I CHANGE IT FROM 100 TO 50 BECAUSE '100' CAUSE TIMEOUT WHEN TEST LOCALLY (I DON'T KNOW...)
...
def free_bacon(score):
    ...
    def cal_length(pi_rest):
        ...
        """
        max = 0
        while not(pi_rest >= pow(10, max) and pi_rest < pow(10, max+1)):
            max += 1
        return max + 1
    
    # 修剪 PI
    while cal_length(pi) > score + 1:
        pi = pi // 10 
    # END PROBLEM 2
...
def swine_align(player_score, opponent_score):
    ...
    def cal_gcd(a, b):
        """返回 A, B 的最大公约数"""
        gcd = min(a, b)
        while gcd > 1 and (a % gcd != 0 or b % gcd != 0):
            gcd -= 1
        return gcd
    
    if cal_gcd(player_score, opponent_score) >= 10 and (player_score * opponent_score) != 0:
        return True
    else:
        return False
    # END PROBLEM 4a
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while True:
        ...
        while who == 0:
            ...
            score0 += take_turn(times, score1, dice)
        ...
        while who == 1:
            ...
            score1 += take_turn(times, score0, dice)
            say = say(score0, score1)
            if score1 >= goal:
                return score0, score1
...
def announce_lead_changes(last_leader=None):
    ...
    def say(score0, score1):
        ...
        # The return function is also a function
        # which takes current leader index as argument
        # and will trun to be "last_leader" when next call
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1): 
        # Focus on player 0
        if who == 0:
            new_get = score0 - last_score
            if new_get > running_high:
                print(str(new_get) + " point(s)! The most yet for Player 0")
                return announce_highest(0, score0, new_get)
            return announce_highest(0, score0, running_high)
        # Focus on player 1
        else:
            new_get = score1 - last_score
            if new_get > running_high:
                print(str(new_get) + " point(s)! The most yet for Player 1")
                return announce_highest(1, score1, new_get)
...
def make_averaged(original_function, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def roll_and_average(*args):
        i, sum = 0, 0
        while i < trials_count:
            i += 1
            sum += original_function(*args)
        return sum / trials_count
    return roll_and_average
    # END PROBLEM 8


def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    """Return the LEAST number of dice (1 to 10) that gives the highest average turn
    ...
    """
    # BEGIN PROBLEM 9
    i, roll_number, highest_points = 1, 1, 0
    while i <= 10:
        result= make_averaged(roll_dice, trials_count)(i, dice)
        if result > highest_points:
            highest_points = result
            roll_number = i
        i += 1
    return roll_number
    # END PROBLEM 9
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(score+free_bacon(opponent_score), opponent_score):
        return 0
    elif not bacon_strategy(score, opponent_score, cutoff):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
