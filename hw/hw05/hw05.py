class VendingMachine:
    ...
    def vend(self):
        ...
        if self.fund == self.price:
            ...
            rest = self.fund - self.price
            self.fund = 0
            return 'Here is your {0} and ${1} change.'.format(self.product, rest)
...
class Coin:
    ...
    def worth(self):
        ...
        if Mint.current_year - self.year > 50:
            return self.cents + Mint.current_year - self.year - 50 # type: ignore
        else:
...
def preorder(t):
    ...
    if t.is_leaf():
        return [t.label]
    return [t.label] + sum((preorder(b) for b in t.branches), []) # type: ignore
