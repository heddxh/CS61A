...
class CallExpr(Expr):
    ...
    def eval(self, env: dict[str, "PrimitiveFunction"]) -> "Value":
        """
        ...
        Number(14)
        """
        operator: Value = self.operator.eval(env)
...
class Number(Value):
    ...
    def apply(self, arguments):
        raise TypeError(
            f"Oof! Cannot apply number {self.value} to arguments {comma_separated(arguments)}"
        )
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments: list[Value]):
        ...
        if len(self.parameters) != len(arguments):
            raise TypeError(f"Oof! Cannot apply number \
                            {comma_separated(self.parameters)} \
                            to arguments {comma_separated(arguments)}")
